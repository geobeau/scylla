...
extern seastar::logger sstreaderlog;
...
public:
    ...
    processing_result process_state(temporary_buffer<char>& data) {
        ...
        case state::START:
            sstreaderlog.trace("{}: pos {} state {} data={}", fmt::ptr(this), current_pos(), state::START, data.size());
            _state = state::KEY_SIZE;
            break;
        case state::KEY_SIZE:
            sstreaderlog.trace("{}: pos {} state {}", fmt::ptr(this), current_pos(), state::KEY_SIZE);
        ...
        case state::KEY_BYTES:
            sstreaderlog.trace("{}: pos {} state {} size={}", fmt::ptr(this), current_pos(), state::KEY_BYTES, this->_u16);
            if (this->_remain != -1) {
                assert(this->_u16 <= this->_remain);
            }
        ...
        case state::POSITION:
            sstreaderlog.trace("{}: pos {} state {}", fmt::ptr(this), current_pos(), state::POSITION);
            if (read_vint_or_uint64(data) != continuous_data_consumer::read_status::ready) {
                _state = state::PROMOTED_SIZE;
                break;
            }
        case state::PROMOTED_SIZE:
            sstreaderlog.trace("{}: pos {} state {}", fmt::ptr(this), current_pos(), state::PROMOTED_SIZE);
            _position = this->_u64;
        ...
        case state::PARTITION_HEADER_LENGTH_1: {
            sstreaderlog.trace("{}: pos {} state {}", fmt::ptr(this), current_pos(), state::PARTITION_HEADER_LENGTH_1);
        ...
        case state::PARTITION_HEADER_LENGTH_2:
            sstreaderlog.trace("{}: pos {} state {} {}", fmt::ptr(this), current_pos(), state::PARTITION_HEADER_LENGTH_2, this->_u64);
            _partition_header_length = this->_u64;
        state_LOCAL_DELETION_TIME:
        case state::LOCAL_DELETION_TIME:
            sstreaderlog.trace("{}: pos {} state {}", fmt::ptr(this), current_pos(), state::LOCAL_DELETION_TIME);
        ...
        case state::MARKED_FOR_DELETE_AT:
            sstreaderlog.trace("{}: pos {} state {}", fmt::ptr(this), current_pos(), state::MARKED_FOR_DELETE_AT);
        ...
        case state::NUM_PROMOTED_INDEX_BLOCKS:
            sstreaderlog.trace("{}: pos {} state {}", fmt::ptr(this), current_pos(), state::NUM_PROMOTED_INDEX_BLOCKS);
        ...
        case state::CONSUME_ENTRY: {
            ...
            sstreaderlog.trace("{}: pos {} state {} size {}", fmt::ptr(this), current_pos(), state::CONSUME_ENTRY, promoted_index_size);
            if (_deletion_time) {
            ...
            if (promoted_index_size <= data_size) {
                ...
                data.trim(0);
                sstreaderlog.trace("{}: skip {} pos {} state {}", fmt::ptr(this), promoted_index_size - data_size, current_pos(), _state);
                return skip_bytes{promoted_index_size - data_size};
            }
        }
            break;
        }
        sstreaderlog.trace("{}: exit pos {} state {}", fmt::ptr(this), current_pos(), _state);
        return proceed::yes;
    }
...
private:
    void advance_to_end(index_bound& bound) {
        sstreaderlog.trace("index {}: advance_to_end() bound {}", fmt::ptr(this), fmt::ptr(&bound));
    ...
    future<> advance_to_page(index_bound& bound, uint64_t summary_idx) {
        sstreaderlog.trace("index {}: advance_to_page({}), bound {}", fmt::ptr(this), summary_idx, fmt::ptr(&bound));
        ...
        if (bound.current_list && bound.current_summary_idx == summary_idx) {
            sstreaderlog.trace("index {}: same page", fmt::ptr(this));
            return make_ready_future<>();
        }

        auto& summary = _sstable->get_summary();
        if (summary_idx >= summary.header.size) {
            sstreaderlog.trace("index {}: eof", fmt::ptr(this));
        ...
        auto loader = [this, &bound] (uint64_t summary_idx) -> future<index_list> {
            ...
            return do_with(std::make_unique<reader>(_sstable, _permit, _pc, _trace_state, position, end, quantity), [this, &bound, summary_idx] (auto& entries_reader) {
                sstreaderlog.trace("index {}: loader() for bound {}: reader={}, context={}", fmt::ptr(this), fmt::ptr(&bound), fmt::ptr(&entries_reader), fmt::ptr(&entries_reader->_context));
                return entries_reader->_context.consume_input().then_wrapped([this, &bound, summary_idx, &entries_reader] (future<> f) {
                    std::exception_ptr ex;
                    if (f.failed()) {
                        ex = f.get_exception();
                        sstreaderlog.error("failed reading index for {}: {}", _sstable->get_filename(), ex);
                    }
                    auto indexes = std::move(entries_reader->_consumer.indexes);
                    sstreaderlog.trace("index {}: loader() for bound {}: done parsing, entries={}", fmt::ptr(this), fmt::ptr(&bound), indexes.size());
        ...
        return _index_lists.get_or_load(summary_idx, loader).then([this, &bound, summary_idx] (shared_index_lists::list_ptr ref) {
            ...
            if (sstreaderlog.is_enabled(seastar::log_level::trace)) {
                sstreaderlog.trace("index {} bound {}: page:", fmt::ptr(this), fmt::ptr(&bound));
                for (const index_entry& e : *bound.current_list) {
                    ...
                    sstreaderlog.trace("  {} -> {}", dk, e.position());
                }
            }
        });
    }
    ...
    future<> advance_to_next_partition(index_bound& bound) {
        sstreaderlog.trace("index {} bound {}: advance_to_next_partition()", fmt::ptr(&bound), fmt::ptr(this));
    ...
    future<> advance_to(index_bound& bound, dht::ring_position_view pos) {
        sstreaderlog.trace("index {} bound {}: advance_to({}), _previous_summary_idx={}, _current_summary_idx={}",
            fmt::ptr(this), fmt::ptr(&bound), pos, bound.previous_summary_idx, bound.current_summary_idx);

        if (pos.is_min()) {
            sstreaderlog.trace("index {}: first entry", fmt::ptr(this));
            return make_ready_future<>();
        } else if (pos.is_max()) {
            advance_to_end(bound);
            return make_ready_future<>();
        ...
        sstreaderlog.trace("index {}: summary_idx={}", fmt::ptr(this), summary_idx);
        ...
        return advance_to_page(bound, summary_idx).then([this, &bound, pos, summary_idx] {
            sstreaderlog.trace("index {}: old page index = {}", fmt::ptr(this), bound.current_index_idx);
            ...
            if (i == std::end(entries)) {
                sstreaderlog.trace("index {}: not found", fmt::ptr(this));
            ...
            bound.element = indexable_element::partition;
            bound.end_open_marker.reset();
            sstreaderlog.trace("index {}: new page index = {}, pos={}", fmt::ptr(this), bound.current_index_idx, bound.data_file_position);
            return make_ready_future<>();
        });
    }
    ...
    future<> advance_upper_past(position_in_partition_view pos) {
        sstreaderlog.trace("index {}: advance_upper_past({})", fmt::ptr(this), pos);
        ...
        if (!e.get_promoted_index()) {
            sstreaderlog.trace("index {}: no promoted index", fmt::ptr(this));
        ...
        return pi.cursor().probe_upper_bound(pos).then([this, &e] (std::optional<clustered_index_cursor::offset_in_partition> off) {
            ...
            sstreaderlog.trace("index {} upper bound: skipped to cell, _data_file_position={}", fmt::ptr(this), _upper_bound->data_file_position);
            return make_ready_future<>();
        });
    }
...
public:
    index_reader(shared_sstable sst, reader_permit permit, const io_priority_class& pc, tracing::trace_state_ptr trace_state)
        ...
        , _trace_state(std::move(trace_state))
    {
        sstreaderlog.trace("index {}: index_reader for {}", fmt::ptr(this), _sstable->get_filename());
    }
    ...
    future<> advance_to(position_in_partition_view pos) {
        sstreaderlog.trace("index {}: advance_to({}), current data_file_pos={}",
        ...
        if (!partition_data_ready()) {
            return read_partition_data().then([this, pos] {
                sstreaderlog.trace("index {}: page done", fmt::ptr(this));
        ...
        return pi.cursor().advance_to(pos).then([this, &e] (std::optional<clustered_index_cursor::skip_info> si) {
            if (!si) {
                sstreaderlog.trace("index {}: position in the same block", fmt::ptr(this));
                return;
            }
            ...
            sstreaderlog.trace("index {}: skipped to cell, _data_file_position={}", fmt::ptr(this), _lower_bound.data_file_position);
        });
    }
