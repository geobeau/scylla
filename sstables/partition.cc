...
logging::logger sstreaderlog("sstable_reader");
...
private:
    ...
    future<> advance_to_next_partition() {
        sstreaderlog.trace("reader {}: advance_to_next_partition()", fmt::ptr(this));
        _before_partition = true;
        auto& consumer = _consumer;
        if (consumer.is_mutation_end()) {
            sstreaderlog.trace("reader {}: already at partition boundary", fmt::ptr(this));
    ...
    future<> read_from_index() {
        sstreaderlog.trace("reader {}: read from index", fmt::ptr(this));
        auto tomb = _index_reader->partition_tombstone();
        if (!tomb) {
            sstreaderlog.trace("reader {}: no tombstone", fmt::ptr(this));
    ...
    future<> read_next_partition() {
        sstreaderlog.trace("reader {}: read next partition", fmt::ptr(this));
